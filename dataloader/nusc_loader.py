...
from pre_processing.dbse import DBSE


class NuScenesloader:
    def __init__(self, detection_path, first_token_path, nusc,config):
        """
    ...
    def __getitem__(self, item) -> dict:
        ...
        # DBSE
        if self.config['preprocessing']['DBSE']:
            ori_dets = DBSE(self.nusc,curr_token,ori_dets)
        
        try:
            list_dets, np_dets = dictdet2array(ori_dets, 'translation', 'size', 'velocity', 'rotation',
                                               'tracking_score', 'tracking_name')
        except:
            list_dets, np_dets = dictdet2array(ori_dets, 'translation', 'size', 'velocity', 'rotation',
