...
from motion_module.kalman_filter import LinearKalmanFilter, ExtendKalmanFilter,IMMFilter
...
class Trajectory:
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict):
        ...
        assert KF_type in ['LinearKalmanFilter', 'ExtendKalmanFilter', 'IMMFilter'], "must use specific kalman filter"
    ...
    def state_predict(self, timestamp: int) -> None:
        ...
        """
        self.timestamp = timestamp
        # if self.class_label in [0,3]:
        if self.cfg['life_cycle']['algorithm'][self.class_label] == 'CB':
        ...
        elif self.cfg['life_cycle']['algorithm'][self.class_label] == 'DW':
            self.motion_management.predict(timestamp)
            self.score_management.predict(timestamp, self.motion_management[timestamp])
        else:
